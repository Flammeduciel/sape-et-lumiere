...
function programmeRoutes(db) {
  ...
  router.post('/', (req, res) => {
    const { day, time, title, icon, description, sort_order } = req.body;
    if (!day || !time || !title) {
      return res.status(400).json({ error: 'day, time et title requis' });
    }
    const result = db.prepare(
      'INSERT INTO programme (day, time, title, icon, description, sort_order) VALUES (?, ?, ?, ?, ?, ?)'
    ).run(day, time, title, icon || 'event', description || '', sort_order || 0);
    res.status(201).json({ id: result.lastInsertRowid, day, time, title, icon, description, sort_order });
  });
  ...
  router.put('/:id', (req, res) => {
    ...
    db.prepare(
      'UPDATE programme SET day = ?, time = ?, title = ?, icon = ?, description = ?, sort_order = ? WHERE id = ?'
    ).run(
      day ?? existing.day, time ?? existing.time, title ?? existing.title,
      icon ?? existing.icon, description ?? existing.description,
      sort_order ?? existing.sort_order, req.params.id
    );
    res.json({ id: Number(req.params.id), day, time, title, icon, description, sort_order });
  });
